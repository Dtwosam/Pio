...
REFERENCE_PATCH = (
    ...
    / "patches"
    / "phase2-single-slot-stack-state-reader.patch"
)
...
def _require_pinned_patch(
    ...
    if not reference_patch.is_file():
        raise ValueError(
            f"reviewed combined Phase-2 single-slot stack reference patch is missing: {reference_patch}"
        )
    if patch.read_bytes() != reference_patch.read_bytes():
        raise ValueError(
            "patch bytes do not match the reviewed combined Phase-2 single-slot stack reference patch"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Preflight or selectively apply the combined Phase-2 single-slot patch that modifies only "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "bundled reviewed combined Phase-2 single-slot stack patch."
        ),
    )
