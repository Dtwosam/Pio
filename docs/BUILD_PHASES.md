...
- post-write cohort evaluation failure isolation in the Phase 9 source-capture pass: post-history/final cohort errors are reported as partial-run failures, bounded fallback targeting may use the last known cohort, and `automatic_source_ready` must remain false after any such evaluation failure
...
- live-vs-historical CLI dispatch correctness: omitting `--as-of` preserves true current-state Phase 8/9 audits instead of synthesizing a historical cutoff, while explicit historical Phase 9 plans/handoffs are inspection-only with collector commands, current templates and execution follow-ups suppressed
...
- live Phase 9 evidence status now exposes per-target wallet historical-backfill exhaustion, pages scanned and scan watermark; exhausted history remains retriable for genuinely new live/current activity, while historical `as_of` views leave these mutable operational fields unset
- wallet candidate/owner backlog accounting plus a fail-closed `WAITING_SOURCE_ACTIVITY` terminal state: the runner may classify a clean wallet pass as waiting only when historical backfill is exhausted, current discovery is not truncated, no owner/position work is deferred, all selected candidates were attempted, no collector failed, thresholds remain unmet, and event/user counts did not advance
...
- exact next eligible history-capture timestamp for cadence-blocked pools, propagated through bounded evidence-run output and immutable maintenance history so unattended retries are scheduled from persisted observation time rather than guessed ETAs
...
- cadence-aware maintenance health that consumes persisted `next_retry_at` from `WAITING_INTERVAL` runs, keeps legitimate long waits healthy until that retry watermark plus the configured health age window, and still marks overdue waits stale
