...
- post-write live evaluation watermarks in the Phase 9 source-capture pass: API ranking is frozen only after API refresh, while post-chain and final cohort readiness advance to later cutoffs so rows persisted by the same run are immediately visible to its own readiness report
