    ...
    with storage.connect() as conn:
        for row in qualified:
            ...
            expected_pool_sha = str(
                evidence.get("pool_snapshot_sha256", "")
            ).strip()
            ...
            pool_record = mint_risk_pool_source_record(pool_source)
            if str(pool_record["pool_address"]) != str(
                row["pool_address"]
            ):
                return False
            if (
                mint_risk_source_sha256(pool_record)
                != expected_pool_sha
            ):
                return False
            ...
            for item in assessments:
                ...
                try:
                    ...
                    return False
                expected_mint_sha = str(
                    item.get("mint_snapshot_sha256", "")
                ).strip()
                if not expected_mint_sha:
                    return False
                ...
                if mint_source is None:
                    return False
                mint_record = mint_risk_mint_source_record(
                    mint_source
                )
                if (
                    str(mint_record["mint_address"])
                    != str(item.get("mint_address", ""))
                ):
                    return False
                if (
                    str(mint_record["observed_at"])
                    != str(item.get("observed_at", ""))
                ):
                    return False
                if (
                    mint_risk_source_sha256(mint_record)
                    != expected_mint_sha
                ):
                    return False
    return True
