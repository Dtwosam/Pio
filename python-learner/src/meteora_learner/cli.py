...
def main() -> None:
    ...
    phase8_evidence_status = subparsers.add_parser(
        "phase8-evidence-status",
        help="Show consolidated Phase 8 continuous-learning, live-label, cycle and promotion readiness",
    )
    ...
    phase8_evidence_status.add_argument(
        "--require-ready",
        action="store_true",
        help="Exit non-zero unless the current Phase 8 promotion gate passes",
    )
    phase8_evidence_status.add_argument(
        "--require-current",
        action="store_true",
    ...
    phase8_evidence_plan = subparsers.add_parser(
        "phase8-evidence-plan",
        help="Show the ordered Phase 8 continuous-learning evidence blockers and next operator step",
    )
    ...
    phase9_evidence_run = subparsers.add_parser(
        "phase9-evidence-run",
        help="Run bounded planner-selected Phase 9 evidence steps until ready, manual input, cadence wait, failure or no-progress",
    )
    ...
    phase2_cycle.add_argument(
        "--pool",
        required=True,
        help="Meteora DLMM pool address",
    )
    phase2_cycle.add_argument(
        "--executor",
        default="/opt/pio/rust-executor/target/release/meteora-executor",
    ...
    if args.command == "phase8-evidence-plan":
        settings = Settings.from_env()
        storage = Storage(settings.database_path)
        result = build_phase8_evidence_plan(
            storage,
            learning_criteria=ContinuousLearningCriteria(
                min_new_chain_observations=(
                    args.min_new_chain_observations
    ...
    if args.command == "phase8-evidence-status":
        settings = Settings.from_env()
        storage = Storage(settings.database_path)
        result = evaluate_phase8_evidence_status(
            storage,
            learning_criteria=ContinuousLearningCriteria(
                min_new_chain_observations=(
                    args.min_new_chain_observations
    ...
    if args.command == "phase9-evidence-run":
        ...
        if not lease.acquired:
            ...
            return
        try:
            result = run_phase9_evidence_until_blocked(
                storage,
                settings=settings,
                criteria=Phase9ResearchBundleCriteria(
                    min_mint_risk_pools=args.min_mint_risk_pools,
