...
def save_position(
    ...
    *,
    ...
    reward_one = (share >> 64) * int(checkpoint_x) >> 64
            ...
            "bins": [
                {
                    ...
                    "bin_reward_per_token_stored": [str(checkpoint_x), "0"],
...
def test_corpus_gate_passes_only_with_exact_amount_and_fee_math(tmp_path):
    ...
    assert report.reward_intervals_exact == 1
    assert report.reward_bins_with_checkpoint_growth == 1
    ...
    assert report.reward_exact_rate == 1.0
...
def test_corpus_validates_all_consecutive_fee_intervals(tmp_path):
    ...
    assert report.reward_intervals_eligible == 2
    assert report.reward_intervals_exact == 2
    assert report.reward_bins_with_checkpoint_growth == 2
    assert report.fee_exact_rate == 1.0
    assert report.reward_exact_rate == 1.0
    assert report.strict_math_gate_passed is True
...
def test_corpus_reward_mismatch_blocks_strict_gate(tmp_path):
    db = tmp_path / "pio.db"
    storage = Storage(db)
    save_position(storage, "a", "2026-09-22T00:00:00+00:00")
    save_position(
    ...
    assert report.reward_intervals_eligible == 1
